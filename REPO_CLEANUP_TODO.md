...
- [x] Update imports in `psannlm/train.py` and related files to be explicit about the dependency on `psann` (keep `from psann.lm...` imports and treat `psann.lm` as part of the public API).
...
- [x] Keep LM-related heavy dependencies behind the `lm` optional extra in the root `pyproject.toml` so `pip install psann` stays light, while `pip install psann[lm]` or `pip install psannlm` pulls in the LM stack.
...
- [x] Add type hints to public APIs in `src/psann` and `src/psann/lm` where feasible, and wire in `mypy` or `pyright` (optionally as a dev extra) to catch interface regressions. (Public surfaces are annotated and `mypy` is now part of the `dev` extra with a baseline `mypy.ini` configuration.)
- [x] Search for duplicated logic between training code (`psann.training`, `psann.lm.train`, `psannlm.train`) and refactor shared pieces into reusable utilities to avoid drift. (Audit complete; estimator training and LM training use distinct helpers, with no obvious duplication worth refactoring right now.)
...
- [x] Add or update tests in `tests/` to cover the main estimators, HISSO pipelines, and LM APIs (at least smoke tests that import and run minimal flows), and ensure CI runs them. (`tests/` already contains broad estimator/HISSO coverage plus `tests/lm/` smoke tests for `psann.lm` public APIs, trainer, and tokenizer/dataset wiring.)
...
- [x] Update `README.md` so the "Getting started" section explicitly distinguishes:
  ...
  - LM/training add-on: `pip install psann psannlm` (or source install with `pip install -e .[lm]`).
...
- [x] Document optional extras in the README, including what each extra (`dev`, `viz`, `sklearn`, `compat`, and LM-related) pulls in (LM deps are called out under `psann[lm]`).
- [x] Add a short "Architecture / repo layout" section to explain `src/psann`, `src/psann/lm`, `psannlm/`, `scripts/`, `benchmarks/`, `notebooks/`, and how they relate (see the "Package layout and tooling" section in the README).
...
- [x] Update the README "Language modeling (PSANN-LM)" section so that:
  - It recommends `pip install psann psannlm` for typical users (and keeps `pip install -e .[dev,lm]` only as the from-source/development path).
  - It makes clear that the high-level APIs live under `psann.lm` (`psannLM`, `psannLMDataPrep`) while the training CLI is provided by the separate `psannlm` package.
...
- [x] Add an "Architecture / repo layout" subsection to the README that briefly explains `src/psann` (core), `src/psann/lm` (LM module), `psannlm/` (standalone LM package), `scripts/`, `benchmarks/`, and `notebooks/`, and how they relate.
