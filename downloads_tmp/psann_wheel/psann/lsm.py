    ...
    def __init__(
        ...
        *,
        ...
        if user_set_hidden_width and not user_set_hidden_units:
            warnings.warn(
                "`hidden_width` is deprecated; use `hidden_units` instead.",
                DeprecationWarning,
                stacklevel=2,
            )
        if user_set_hidden_units and user_set_hidden_width:
            try:
                mismatch = int(hidden_units) != int(hidden_width)
            except Exception:
                mismatch = hidden_units != hidden_width
            if mismatch:
                warnings.warn(
                    "`hidden_units` overrides `hidden_width` because the values differ.",
                    UserWarning,
                    stacklevel=2,
                )
        width_val = hidden_units if user_set_hidden_units else hidden_width
...
class LSMConv2d(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        user_set_conv_channels = conv_channels is not None
        user_set_hidden_channels = hidden_channels is not None
        if user_set_hidden_channels and not user_set_conv_channels:
            warnings.warn(
                "`hidden_channels` is deprecated; use `conv_channels` instead.",
                DeprecationWarning,
                stacklevel=2,
            )
        if user_set_conv_channels and user_set_hidden_channels:
            try:
                mismatch = int(conv_channels) != int(hidden_channels)
            except Exception:
                mismatch = conv_channels != hidden_channels
            if mismatch:
                warnings.warn(
                    "`conv_channels` overrides `hidden_channels` because the values differ.",
                    UserWarning,
                    stacklevel=2,
                )
        channels_val = conv_channels if user_set_conv_channels else hidden_channels
