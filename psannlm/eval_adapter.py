...
Usage example:
  ...
  lm_eval --model custom \
    --model_args \
      custom_model_class=psannlm.eval_adapter.PSANNLM,\
...
class PSANNLM(LM):
    ...
    def __init__(
        ...
        *,
        ...
        # Allow direct repo-checkout usage without requiring editable installs.
            ...
            for name in candidates:
                try:
                    path = hf_hub_download(
                        repo_id=hf_repo, filename=name, revision=hf_revision, token=hf_token
                    )
                    local_ckpt = path
                    break
