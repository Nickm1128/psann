...
from psannlm.data_loader import build_stream_loader
from psannlm.data_stream import streamed_token_iterator
...
from .tokenizer import _prepare_tokenizer
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if use_streaming:
        ...
        if resume_step > 0:
            skip_sequences = (
                int(resume_step) * int(micro_batch) * max(1, int(args.grad_accum_steps))
            )
            if rank == 0:
                print(
