...
Progress: 70 / 189 bullets complete (37.0%) - update this tally whenever checklist statuses change.
...
- 70 checklist bullets now carry completion stamps out of 189 tracked items (~37.0%); the HISSO logging CLI, config templates, docs refresh, new `with_conv_stem` helper, dataloader shuffle guidance, and HISSO dataset alignment land ahead of the final GPU validation pass.
...
- Update README and TECHNICAL_DETAILS where behavior or names change; ensure notebooks reflect the stable API. (partial 2025-10-28; core docs refreshed, notebooks pending API alignment)
  - Subtasks
    - Patch docs for alias deprecations and estimator duplication removal.
    - Add minimal, deterministic examples for `WaveResNetRegressor`, `ResPSANNRegressor`, `ResConvPSANNRegressor`.
    - Update TODO progress; add summary to Notes. (done 2025-10-28)
...
- Completed deliverables:
  ...
  - HISSO logging dataset loaders now emit primary-aligned price matrices, unblocking the dense baseline CLI run (`src/psann/scripts/hisso_log_run.py`), with coverage via `tests/test_hisso_logging_cli.py`.
...
- Session handoff (2025-10-29 prep):
  ...
  - HISSO logging CLI implemented (`src/psann/scripts/hisso_log_run.py`) with CPU smoke coverage; pending work is staging datasets for the GPU runpod sweep (`configs/hisso/wave_resnet_small.yaml` expects `datasets/wave_resnet_small.npz` on the remote node).
...
- Regression coverage status:
  ...
  - Remaining gaps: model-specific forward tests (WaveResNet/conv/dense cores) under CUDA, mixed-precision training loops, and the GPU metrics handoff for docs/notebooks once runpod runs complete (`dense_cpu_smoke` passes locally after price alignment; GPU run waits on dataset staging).
...
- Next focus areas:
  1. Refresh notebooks/Colab pointers with the new logging CLI after the GPU metrics land (blocked by dataset bug in `configs/hisso/dense_cpu_smoke.yaml`).
  2. Final step: execute the deferred runpod GPU session (HISSO validation, CUDA smoke, metric capture) and sync artifacts back before sign-off; dense baseline run currently fails with `allocations/prices must be (T,M)` and needs dataset adjustment prior to rerun.
