...
Progress: 84 / 189 bullets complete (44.4% simple; 76.7% complexity-weighted) — update this tally whenever checklist statuses change.
...
**Next Session Pickup (2025-11-02)**
- Decisions received (unblock items):
  - Default logging directory (Colab vs local shells): No preference (Nick). Action: document recommended defaults and keep explicit `--output-dir` in CLI help.
  - “Extras” datasets: Archive (Nick). Action: mark as archived in the compendium and remove them from active sweeps.
  - Mixed-precision guidance: Approved (Nick). Action: finalize wording in docs; mark board Blocked item resolved.
- Immediate tasks to apply decisions:
  - Update CLI/Docs: add a short note in `docs/HISSO_logging_spec.md` and CLI help recommending local `runs/hisso/` and Colab `/content/hisso_logs/` as defaults when users don’t have a preference; keep `--output-dir` explicit.
  - Compendium: add a compact GPU sweep summary table and mark “extras” datasets as archived (link location), leaving only primary datasets in active sections.
  - Notebook: add the 212855/153117 Runpod metrics block to `notebooks/HISSO_Logging_GPU_Run.ipynb` for parity with the walkthrough.
  - Board hygiene: move the three Blocked items to resolved with a one-liner (decisions above), add a Session Log line, and recompute Autosummary if any boxes change.
- Validation:
  - CPU quick: `PYTHONPATH=src python -m pytest -q -m "not slow"`
  - Optional CUDA (if available): `python -m pytest -q -k cuda -m "not slow"` and `python -m pytest -q tests/test_training_loop.py::test_training_loop_early_stopping_runs_on_cuda`
- Commit template:
  - `docs+board: finalize AMP guidance; archive extras; add GPU sweep summary; logging-dir guidance`
