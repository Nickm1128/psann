...
def test_release_metadata_dependency_and_version_agreement():
    ...
    assert core["version"] == lm["version"] == psann.__version__ == "0.13.0"
    assert core["requires-python"] == lm["requires-python"] == ">=3.9"
    assert {"numpy>=1.23", "torch>=2.1", "PyYAML>=6.0"} <= set(core["dependencies"])
    assert {
        "psann>=0.13.0", "numpy>=1.23", "torch>=2.1", "PyYAML>=6.0",
        "sentencepiece>=0.1.99", "tokenizers>=0.13", "datasets>=2.14",
...
@pytest.mark.parametrize("module", ["psann", "psann.sklearn", "psann.estimators", "psannlm", "psannlm.lm"])
def test_wildcard_exports_resolve_and_exclude_legacy_names(module):
    ...
    legacy = {"ResPSANNRegressor", "ResConvPSANNRegressor", "SGRPSANNRegressor",
              "WaveResNetRegressor", "GeoSparseRegressor", "psannLM", "psannLMDataPrep"}
    assert not legacy.intersection(namespace)
    if module == "psann":
        assert not {"ActivationConfig", "AttentionConfig", "StateConfig"}.intersection(namespace)
        assert namespace["ArchitectureConfig"] is importlib.import_module("psann.architectures").ArchitectureConfig


@pytest.mark.parametrize("kind,legacy", [("wave", "waveresnet"), ("residual", "respsann")])
def test_parameter_count_cli_preserves_nondefault_graph_logits_and_gradients(kind, legacy, monkeypatch, capsys):
    ...
    args = ["--vocab-size", "37", "--d-model", "24", "--n-layers", "2", "--n-heads", "3",
            "--d-mlp", "36", "--pos-enc", "alibi", "--wave-interleave", "--wave-kernel-size", "5",
            "--wave-dilation-growth", "2", "--wave-dropout", "0.0"]
...
@pytest.mark.parametrize("kind,legacy", [("wave", "waveresnet"), ("residual", "respsann")])
def test_kv_benchmark_canonical_and_compatibility_routes_generate_identically(kind, legacy, monkeypatch):
    ...
    for base in (legacy, kind):
        ...
        outputs.append(consumer.run_benchmark(consumer.BenchmarkConfig(
            batch_size=2, prompt_length=24, max_new_tokens=3, base=base,
            d_model=24, n_layers=2, n_heads=3, tokenizer="simple",
            positional_encoding="alibi", device_mode="cpu",
        )))
    assert models[0].config == models[1].config
    assert outputs[0]["fast_path"]["sample_output_preview"] == outputs[1]["fast_path"]["sample_output_preview"]
    ids = torch.arange(14).reshape(2, 7)
